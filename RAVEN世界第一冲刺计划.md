...
> 创建时间：2026-06-25
> 最后修订：2026-06-28（v8：H20 实测推翻虚假基线，Phase 1 否决令解除，新增 SQ8/多线程/自适应ef/预取调优）
...
#### Phase 0D：预取参数自动调优（v8 新增）🔴 P0

> **确定性收益，一天内可完成。** Glass Optimize() 在 SIFT1M 上获得 30.13% 性能提升。
> RAVEN 的 po 硬编码为 8，Glass 实测最优 po=2。从未扫描过。

| 编号 | 任务 | 文件 | 预期收益 |
|:--|:--|:--|:--|
| 0D.1 | **po/pl 参数空间扫描**：在 ef=50/100 下扫描 po∈{0,1,2,4,8,16,32}，找最优配置 | `ef_po_sweep.rs`（已有骨架） | +20-30% QPS |
| 0D.2 | **实现 `optimize_prefetch()` 方法**：借鉴 Glass Optimize()，自动扫描 (po, pl) 参数空间 | `vamana.rs` GraphSearcher | 工程化 |
| 0D.3 | **按 ef_search 自适应 po**：po=8 适合 ef=50，po=4 适合 ef=100（消融实验已有数据） | `vamana.rs` | 不同工作点自动最优 |

**实施方式**：
```
fn optimize_prefetch(&mut self, sample_queries: &[f32]) {
    for po in [0, 1, 2, 4, 8, 16, 32] {
        self.prefetch_offset = po;
        let qps = self.benchmark(sample_queries);
        if qps > best_qps { best_po = po; }
    }
    self.prefetch_offset = best_po;
}
```
...
| 子步骤 | 预期效果 | 是否单独满足规则 2 |
|:--|:--|:--|
| 0a. 实现 SQ8 编码/解码 | 向量 128B → 32B，内存降 4x | 否（中间态） |
| 0b. 实现 SQ8 L2 距离核 | 标量 u8 距离计算，比 f32 快 ~1.5x | 否（中间态） |
| 0c. 接入搜索路径 + rerank | recall 恢复（SQ8 量化误差小） | 否（需调参） |
| 0d. **终态评估**：recall ≥ 0.95, QPS ≥ baseline × 1.3 | **是** | **终态门** |

**SQ8 预期收益**：
- 距离计算快 ~1.5x（u8 运算 vs f32）
- 内存带宽降 4x（32B/向量 vs 128B/向量）
- 端到端 QPS 预期 +30-50%（Amdahl 稀释后）
- recall 损失小（SQ8 量化误差远小于 4-bit PQ）
...
| **LUT16 SIMD ADC (端到端 QPS)** | — | ~15,000-20,000 | **预计 2-3x（Amdahl 稀释后）** | 取决于距离计算占比 |

> **rerank 的带宽陷阱**：4-bit LUT16 的"带宽降 4 倍"优势**只在图导航阶段成立**。rerank 阶段仍然要读全精度 f32 向量。
...
```rust
// 方案 A：基于候选集质量的提前终止
fn adaptive_search(query, max_ef):
    ef = max_ef / 4  // 从小开始
    loop {
        results = greedy_search(query, ef)
        if results.convergence_ratio() > threshold || ef >= max_ef:
            return results
        ef *= 2  // 不够就加倍
    }

// 方案 B：基于入口点距离的 ef 预测
fn predict_ef(entry_distance, hist):
    // 入口距离远的查询通常更难
    if entry_distance < hist.p25 { ef = 20 }
    else if entry_distance < hist.p75 { ef = 50 }
    else { ef = 100 }
```

**预期收益**：
- 在 recall≥95% 的工作点上，avg_visited 降低 20-40%
- QPS 提升 20-40%（因为 avg_visited 直接决定距离计算次数）
- 不需要改任何图结构，纯搜索策略优化
...
| recall@10 | 当前 QPS (f32 单线程) | +预取调优 | +SQ8 | +4-bit PQ | +多线程 (8核) | vs 榜首 |
|:--|:--|:--|:--|:--|:--|:--|
| 0.95 | ~10,000 | ~13,000 | ~15,000 | ~20,000 | ~100,000+ | 超越榜首 |
| 0.97 | 8,657 | ~11,000 | ~13,000 | ~17,000 | ~80,000+ | 超越榜首 |
| 0.99 | 4,479 | ~5,800 | ~6,500 | ~8,000 | ~50,000+ | 可比 |
...
| 🔴 **P0** | **0D** | **预取参数自动调优** | **+20-30% QPS（确定性）** | **1天** | 🟢 低 | **Glass 实测 30% 收益。po=8→po=2** |
| 🔴 **P0** | **1.0** | **Phase 1 Step 0: SQ8 量化** | **+30-50% QPS** | **2天** | 🟡 中 | 过渡验证，降低 4-bit 风险 |
...
| 🟡 P1 | 4.5 | 自适应 ef | +20-40% QPS | 2-3天 | 🟡 中 | 差异化创新 |
...
9. 🟡 **Phase 2: rerank 精调** + **Phase 4.5: 自适应 ef**（并行推进）
...
17. **自适应 ef** → P1（RAVEN 独有，Glass/DiskANN 均无）
