...
> 创建时间：2026-06-25
> 最后修订：2026-06-26（v4：终态门改为分层量化判定 + rerank 带宽优势仅覆盖图导航段）
...
| recall@10 (α=1.2, l_build=100, r_max=32, ef=100) | 0.9517 | **待测** | quick_recall_check |
| QPS (同上参数) | 1,287 | **待测** | quick_recall_check |
| 建图时间 | 1,054.8s | **待测** | quick_recall_check |
| **avg_visited**（每查询平均距离计算次数） | 未测 | **必须测** | 图质量核心指标，见 §1.3 |
...
| **avg_visited @ recall 0.95** | **必须测** | 100-300（公开数据） | 待对比 |
| ADC 路径 vs f32 | ADC **更慢** | 榜首 ADC 快 3-5x | **根本性缺陷** |
| 建图时间 | ~790s (r_max=32) | ~200-500s | 2-4x |

**核心发现：RAVEN 的 ADC 路径比 f32 还慢，这是反直觉的。** 世界顶尖库（ScaNN 等）之所以快，正是因为 PQ-ADC 路径比 f32 快 3-5 倍。RAVEN 有量化器但搜索时没有用 SIMD lookup table 加速 ADC，而是逐子空间标量计算——这等于浪费了量化的全部优势。
...
| 判定等级 | 条件 | 处置 |
|:--|:--|:--|
| ✅ 成功 | recall@10 ≥ 0.95 **且** 端到端 QPS ≥ f32 基线 × 1.5 | 并入主线 |
| ⚠️ 部分成功 | recall@10 ≥ 0.95 **且** 端到端 QPS ∈ [f32 基线 × 1.0, f32 基线 × 1.5) | **不自动并入主线**；触发 profiler 时间分解复盘，找出 Amdahl 瓶颈，决定是否继续优化 |
| ❌ 失败 | recall@10 < 0.95 **或** 端到端 QPS < f32 基线 | 整体回退，记录原因 |
...
1. **建立干净基线** → 确认无后台进程，重测 QPS/recall/建图时间/**avg_visited**
...
**RAVEN 距离世界第一的核心差距很可能在搜索热路径的距离计算方式：当前用 f32 全精度算距离，而顶尖库用 LUT16-shuffle（pshufb + maddubs）加速的 PQ-ADC 实现了 3-5 倍的距离计算加速（端到端受 Amdahl 稀释）。补上这个差距（Phase 1，采用 4-bit PQ + 寄存器内查表，放弃 gather），再叠加 rerank 补偿 recall，RAVEN 有实力在 SIFT1M 上挑战 ann-benchmarks Pareto 前沿。但在此之前必须先做三件事：（1）建立干净环境下的真实基线，包括 avg_visited 指标来验证"图算法已足够好"这个假设；（2）修复 ann-benchmarks wrapper 每次重建索引的致命 bug（S1）；（3）拉取真实排行榜数据，避免瞄错靶子。否则一切性能数据和分析都建立在沙子上。**
