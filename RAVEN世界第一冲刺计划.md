...
| 0C.3 | D2 ✅ | **BuildMetadata 集成到序列化**：`serialize()` 写入 metadata TOML trailer（FLAG_HAS_METADATA），`deserialize()` 读回。向后兼容旧格式。4 字节对齐填充确保 CSR cast_slice 安全 | `vamana.rs`, `metadata.rs`, `serialize.rs` | ✅ 已完成 | F.7 要求，已修复 |
...
6. ✅ **0C.3 BuildMetadata 落盘** + ✅ **0C.5 RP-Tuning alpha 范围**（均已完成）
