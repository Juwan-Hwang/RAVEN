...
| **GLASS-COMP v2 recall/QPS** | - | recall=0.9703, QPS=4,910 (ef=50) | 200/32/2，建图 609.8s，2026-06-27 重测 |
...
### GLASS-COMP v2 结果（2026-06-27，代码修复后重测）

| ef_search | recall@10 | QPS | avg_visited | p50 | p95 | p99 | max |
|:--|:--|:--|:--|:--|:--|:--|:--|
| 50 | 0.9703 | 4,910 | **1,399.5** | 1,425 | 1,652 | 1,777 | 2,300 |
| 75 | 0.9860 | 5,113 | 1,889.6 | 1,947 | 2,224 | 2,356 | 2,698 |
| 100 | 0.9920 | 4,070 | 2,355.5 | 2,440 | 2,795 | 2,939 | 3,212 |
| 150 | 0.9966 | 2,648 | 3,228.2 | 3,360 | 3,903 | 4,070 | 4,459 |
| 200 | 0.9980 | 2,012 | 4,044.8 | 4,214 | 4,963 | 5,185 | 5,690 |
| 300 | 0.9990 | 1,584 | 5,555.9 | 5,792 | 6,953 | 7,303 | 7,896 |

> **vs v1 对比**：
> - recall@ef=50：0.9473 → 0.9703（+2.3%，修复 init_random_graph 双向边后图质量提升）
> - QPS@ef=50：4,175 → 4,910（+17.6%）
> - avg_visited@ef=50：1,390 → 1,399.5（基本不变，图导航效率未改善）
> - 建图时间：1,574.7s → 609.8s（-61.3%，环境更干净 + 代码修复）
> - 度数：mean=32.0 p95=32 p99=32 max=32 isolated=0（完美均匀，无 overflow）

### 建图时间分析：609.8s vs 预期 400s（+50%）

> **v7 实验结果：VisitedTracker 复用方案已实施，建图时间几乎无改善**
>
> | 版本 | 建图时间 | recall@ef=50 | avg_visited@ef=50 | QPS@ef=50 |
> |:--|:--|:--|:--|:--|
> | v6.4（旧版 greedy_search） | 609.8s | 0.9703 | 1,399.5 | 4,910 |
> | v7（map_init 复用） | 603.3s | 0.9703 | 1,399.5 | 6,811* |
>
> *QPS 波动为环境噪声（ef=75 暴跌至 2,754，avg_visited 完全相同），非真实性能变化。
>
> **2TB 分配流量理论被证伪**：
> - 修改正确（recall/avg_visited/degree 完全一致），但建图仅快 1.1%（6.5s）
> - 原因：Windows 堆分配器在线程内复用同一 1MB 块，实际开销只有 `memset(1MB)`
> - memset 带宽 ~10GB/s/core × 16 核 = 160GB/s，2TB / 160GB/s ≈ 12.5s
> - 消除后省 ~6-12s，不是预期的 ~200s
>
> **真正的建图瓶颈是距离计算**：
> - 2 轮 × 1M 节点 × 每次搜索访问 ~1400 节点 = 2.8B 次距离计算
> - 每次 128-dim L2 = 256 FLOPs → 总计 716 TFLOPs
> - 16 核 × ~50 GFLOPS/core = 800 GFLOPS → ~895s（含非距离开销后 ~600s）
>
> **glass 建图快 1.2-3x 的根因**：HNSW 分层结构使建图时大多数插入只搜索小子图，
> 而非完整 1M 节点 flat 图。这与 avg_visited 10x 差距同根同源——都是 D1（随机层级缺失）。
...
| 0C.4 | — ✅ | **建图路径 VisitedTracker 复用**：已用 rayon `map_init` 实现。建图 609.8s -> 603.3s（-1.1%），**2TB 分配流量理论被证伪**，真正瓶颈是距离计算（2.8B 次 L2） | `vamana.rs` | 🟢 P2 | 已完成，收益远低于预期。建图慢的根因是 D1（flat 图搜索全量节点），不是分配 |
| 0C.5 | D7 | **RP-Tuning alpha 范围对齐 F.4**：`alpha_points` 从 `[1.0, 1.2, 1.5, 2.0]` 扩展到 `[0.8, 1.0, 1.2, 1.5, 2.0, 3.0]` | `rp_tuning.rs` | 🟡 P1 | 正文与附录矛盾 |
...
| ✅ 已完成 | 0C.4 | 建图路径 VisitedTracker 复用 | 建图 609.8s -> 603.3s（-1.1%） | 0.5天 | 🟢 | 2TB 理论被证伪，真正瓶颈是距离计算 |
...
5. ✅ **0C.4 建图 VisitedTracker 复用**（已完成 → 609.8s → 603.3s，-1.1%，2TB 理论被证伪）
6. 🟡 **0C.3 BuildMetadata 落盘** + **0C.5 RP-Tuning alpha 范围**（可并行，各 0.5天）
