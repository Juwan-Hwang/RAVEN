//! 4-bit PQ (K=16) LUT-ADC benchmark (Phase 1 Step 1)
//!
//! 三方对比：f32 全精度 vs SQ8 vs PQ4（均含 f32 rerank）。
//! 建图一次，扫描多个 ef 工作点。
//!
//! 结果自动写入 pq4_bench_result.txt
//!
//! 用法：cargo run --release --bin pq4_bench
...
use raven::quant::{PQ4Dataset, SQ8Dataset};
...
fn bench_mode(
    ...
    for q in 0..nq {
        ...
        let result = match mode {
            ...
            SearchMode::PQ4(_) => searcher.search_pq4(query, k),
        };
...
fn main() {
    ...
    println_both!("=== PQ4 vs SQ8 vs f32 Benchmark (Phase 1 Step 1) ===\n");
    ...
    println_both!(
        "PQ4 编码: {:.1}s ({} bytes, {:.1} MB)",
        t0.elapsed().as_secs_f64(),
        pq4.codes.len(),
        pq4.codes.len() as f64 / 1e6
    );

    println_both!(
        "\n内存对比: f32={:.1} MB | SQ8={:.1} MB ({:.1}x) | PQ4={:.1} MB ({:.1}x)",
        (n * dim * 4) as f64 / 1e6,
        sq8.codes.len() as f64 / 1e6,
        (n * dim * 4) as f64 / sq8.codes.len() as f64,
        pq4.codes.len() as f64 / 1e6,
        (n * dim * 4) as f64 / pq4.codes.len() as f64
    );
    ...
    for &ef in &ef_list {
        ...
        let pq4_r = bench_stable(&train, &graph, &test, dim, nq, &gt, gt_k, k, ef, &SearchMode::PQ4(&pq4));
        let pq4_speedup = pq4_r.qps / f32_r.qps;
        println_both!(
            "  {:>6}  {:>8}  {:>8.4}  {:>12.0}  {:>8.2}x  {:>12.1}  {:>+10.4}  {:>+10.1}",
            ef, "PQ4", pq4_r.recall, pq4_r.qps, pq4_speedup, pq4_r.avg_visited,
            pq4_r.recall - f32_r.recall, pq4_r.avg_visited - f32_r.avg_visited
        );

        // SQ8→PQ4 delta
        println_both!(
            "  {:>6}  {:>8}  {:>8.4}  {:>12.0}  {:>8.2}x  {:>12.1}  {:>+10.4}  {:>+10.1}",
            "", "PQ4/SQ8", pq4_r.recall, pq4_r.qps, pq4_r.qps / sq8_r.qps, pq4_r.avg_visited,
            pq4_r.recall - sq8_r.recall, pq4_r.avg_visited - sq8_r.avg_visited
        );
        println_both!("  {}", "-".repeat(92));
    }
    ...
    let ef50_pq4 = bench_stable(&train, &graph, &test, dim, nq, &gt, gt_k, k, 50, &SearchMode::PQ4(&pq4));
    let ef100_pq4 = bench_stable(&train, &graph, &test, dim, nq, &gt, gt_k, k, 100, &SearchMode::PQ4(&pq4));
    let ef200_pq4 = bench_stable(&train, &graph, &test, dim, nq, &gt, gt_k, k, 200, &SearchMode::PQ4(&pq4));

    let speedup_50 = ef50_pq4.qps / ef50_f32.qps;
    let passes_recall = ef50_pq4.recall >= 0.95 || ef100_pq4.recall >= 0.95 || ef200_pq4.recall >= 0.95;
    let passes_qps = speedup_50 >= 1.5 || ef100_pq4.qps / ef50_f32.qps >= 1.5;

    println_both!("  ef=50:  f32 recall={:.4} QPS={:.0} | PQ4 recall={:.4} QPS={:.0} | speedup={:.2}x",
        ef50_f32.recall, ef50_f32.qps, ef50_pq4.recall, ef50_pq4.qps, speedup_50);
    println_both!("  ef=100: PQ4 recall={:.4} QPS={:.0}", ef100_pq4.recall, ef100_pq4.qps);
    println_both!("  ef=200: PQ4 recall={:.4} QPS={:.0}", ef200_pq4.recall, ef200_pq4.qps);
    ...
    println_both!("    SQ8 ef=50:     recall=~0.965  QPS=~12,000  (Phase 1.0 实测)");

    // 写文件
    let mut f = File::create("pq4_bench_result.txt").expect("create result file");
    f.write_all(out.as_bytes()).expect("write result");
    println!("\n结果已写入 pq4_bench_result.txt");
}
