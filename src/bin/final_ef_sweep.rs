//! 最终 ef sweep — 用 nav_m=32 图生成提交曲线
//!
//! 用法: cargo run --release --bin final_ef_sweep
...
fn main() {
    ...
    eprintln!("=== Final ef sweep (nav_m=32, DirectionalPrune, r_max=32) ===");
    ...
    for &ef in &[40, 45, 50, 55, 60, 65, 70, 80, 100] {
        ...
        for _ in 0..REPEATS {
            for q in 0..nq {
                let query = &test[q * dim..(q + 1) * dim];
                let r = searcher.search_sq8(query, K);
    ...
    std::fs::write("tuning/final_ef_sweep_navm32.csv", csv).expect("write");
    eprintln!("\n结果写入 tuning/final_ef_sweep_navm32.csv");
}
