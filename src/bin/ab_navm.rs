...
fn main() {
    ...
    let g16 =
        VamanaGraph::load(std::path::Path::new("data/sift/sweep_navm16.bin")).expect("load navm16");
    let g32 =
        VamanaGraph::load(std::path::Path::new("data/sift/sweep_navm32.bin")).expect("load navm32");
    ...
    for q in 0..nq {
        ...
        for &g in gt_slice {
            if r16.iter().any(|(id, _)| *id == g as u32) {
                hits16 += 1;
            }
            if r32.iter().any(|(id, _)| *id == g as u32) {
                hits32 += 1;
            }
        }
    }
